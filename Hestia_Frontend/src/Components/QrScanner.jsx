...
const QrScanner = () => {
  ...
  const handleScan = async(result) => {
    ...
    if (qr_code) {
        ...
        try{
            const res = await axios.post("/api/user/scan", {qr_code});
            if(res?.data) {
  ...
  const handleStartScan = () => {
    ...
    console.log("started scanning", hasScanned) // Allow scanning again
  };
  ...
  return(
    <div style={{display: "flex", flexDirection:"column", alignItems: "center"}}>
        <Box component="section" sx={{ border: 1, borderRadius: 2  }} style={ {...box, backgroundColor: scanningState ? "transparent" : "grey"  }}>
        {scanningState ? (
          <QrReader
            onResult={(result, error) => {
                
              if (result && !hasScanned.current){
                console.log("stupid software check: " + result, hasScanned.current)
                console.log(hasScanned)
                if(hasScanned.current === false){
                    console.log("hit")
                    handleScan(result);
                } 
                return
      ...
      {user && (
        <div style={{ marginTop: 16 }}>
          <Typography variant="h6">User Found: {user?.first_name} {user?.last_name}</Typography>
        </div>
      )}
